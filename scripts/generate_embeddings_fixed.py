#!/usr/bin/env python3
"""
Genera embeddings per tutti i prodotti - FIXED VERSION
"""
...
def create_product_text(product: dict) -> str:
    """
    Crea testo completo per embedding con BOOST su categoria
    """
    parts = []
    
    # CATEGORIA RIPETUTA 3 VOLTE per maggior peso!
    ...
    # Tecnologie
    if product.get('tecnologie'):
        tecnologie = product['tecnologie']
        if tecnologie:
            if isinstance(tecnologie[0], dict):
                tech_text = ". ".join([f"{t['titolo']}: {t['descrizione']}" for t in tecnologie])
            else:
                tech_text = ", ".join(tecnologie)
            parts.append("Tecnologie: " + tech_text)
